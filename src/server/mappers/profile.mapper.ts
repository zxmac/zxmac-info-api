import { concatId, generateUUID } from "../helpers/utils";
import { 
...
function mapProfile(cv: CvDto) {
  ...
  profile.experience = cv.experience.map((tech, index) => {
    ...
    exp.guid = concatId('EXP', exp.title)
    ...
    exp.projects = tech.expApps.map((expApp, index2) => {
      const project = {
        ...
        sequence: index2 + 1,
        techStackGuid: generateUUID('TS'),
        lexiconGuid: generateUUID('LX'),
        mediaGuid: generateUUID('MD'),
        experienceId: exp.id!
      } as ProjectDto;

      project.techStackGuid = concatId('TS', exp.title, project.name);
      project.lexiconGuid = concatId('LX', exp.title, project.name);
      project.mediaGuid = concatId('MD', exp.title, project.name);
...
export default { techTypeList, mapProfile }