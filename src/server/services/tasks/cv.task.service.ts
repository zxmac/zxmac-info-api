...
async function processPrivateSync(sheetId: string) {
  const values = await gsheets_api.read.getValues(sheetId);
  return ProfileLibService.mapProfile(GsheetLibService.gsheetToCv(values));
}

async function processPrivateSyncSheets(sheetId: string) {
  const values = await gsheets_api.read.getValues(sheetId);
  ...
  const allSheetProps = await gsheets_api.read.getSheets(sheetId);
  const tableSheets = allSheetProps?.filter(x => x.properties?.title != 'Sheet1');
  if (tableSheets?.length) {
    for (const sheet of tableSheets) {
      await gsheets_api.resource.deleteSheet(sheetId, sheet?.properties?.sheetId || 0);
    }
  }

  for (const sheet of Object.values(sheets)) {
    await gsheets_api.resource.addSheet(sheetId, sheet);
  }
  
  // libs
  const techTypeListValues = SheetLib.listToValues(ProfileLibService.techTypeList);
  await gsheets_api.write.createSheetRows(sheetId, sheets.techTypes, techTypeListValues);
  // end 

  const profileValues = SheetLib.objToValues(profile);
  await gsheets_api.write.createSheetRows(sheetId, sheets.profile, profileValues);

  const profileLinksValues = SheetLib.listToValues(profile.profile_links || []);
  await gsheets_api.write.createSheetRows(sheetId, sheets.profileLinks, profileLinksValues);
  
  const techStackValues = SheetLib.listToValues(profile.tech_stack || []);
  await gsheets_api.write.createSheetRows(sheetId, sheets.techStacks, techStackValues);

  const educationValues = SheetLib.listToValues(profile.education || []);
  await gsheets_api.write.createSheetRows(sheetId, sheets.education, educationValues);
  ...
  await gsheets_api.write.createSheetRows(sheetId, sheets.style, styleValues);
  
  // experience
  const experienceValues = SheetLib.listToValues(profile.experience || []);
  await gsheets_api.write.createSheetRows(sheetId, sheets.experience, experienceValues);

  const expLexiconsValues = SheetLib.listToValues(profile.experience?.flatMap(x => x.lexicons) || []);
  await gsheets_api.write.insertSheetRows(sheetId, sheets.lexicons, expLexiconsValues);

  const expProjValues = SheetLib.listToValues(profile.experience?.flatMap(x => x.projects) || []);
  await gsheets_api.write.insertSheetRows(sheetId, sheets.projects, expProjValues);
  ...
  await gsheets_api.write.insertSheetRows(sheetId, sheets.techStacks, expProjTechStackValues);
  ...
  await gsheets_api.write.insertSheetRows(sheetId, sheets.lexicons, expProjLexiconsValues);

  const expProjMediaValues = SheetLib.listToValues(profile.experience?.flatMap(x => x.projects).flatMap(x => x?.media).filter(x => x) || []);
  await gsheets_api.write.insertSheetRows(sheetId, sheets.media, expProjMediaValues);
